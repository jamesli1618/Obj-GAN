...
class Checkpoint(object):
    ...
    def __init__(self, model, optimizer, epoch, step, cap_word2index, 
        cap_index2word, label_word2index, label_index2word, path=None):
        self.model = model
        self.optimizer = optimizer
        self.cap_word2index = cap_word2index
        self.cap_index2word = cap_index2word
        self.label_word2index = label_word2index
        self.label_index2word = label_index2word
        self.epoch = epoch
        self.step = step
        self._path = path
    ...
    def save(self, experiment_dir):
        ...
        with open(os.path.join(path, self.CAP_WORD2INDEX), 'wb') as fout:
            dill.dump(self.cap_word2index, fout)
        with open(os.path.join(path, self.CAP_INDEX2WORD), 'wb') as fout:
            dill.dump(self.cap_index2word, fout)
        with open(os.path.join(path, self.LABEL_WORD2INDEX), 'wb') as fout:
            dill.dump(self.label_word2index, fout)
        with open(os.path.join(path, self.LABEL_INDEX2WORD), 'wb') as fout:
            dill.dump(self.label_index2word, fout)
    ...
    def load(cls, path):
        ...
        with open(os.path.join(path, cls.CAP_WORD2INDEX), 'rb') as fin:
            cap_word2index = dill.load(fin)
        with open(os.path.join(path, cls.CAP_INDEX2WORD), 'rb') as fin:
            cap_index2word = dill.load(fin)
        with open(os.path.join(path, cls.LABEL_WORD2INDEX), 'rb') as fin:
            label_word2index = dill.load(fin)
        with open(os.path.join(path, cls.LABEL_INDEX2WORD), 'rb') as fin:
            label_index2word = dill.load(fin)
        optimizer = resume_checkpoint['optimizer']
        return Checkpoint(model=model, cap_word2index=cap_word2index,
                          cap_index2word=cap_index2word,
                          label_word2index=label_word2index,
                          label_index2word=label_index2word,
                          optimizer=optimizer,
